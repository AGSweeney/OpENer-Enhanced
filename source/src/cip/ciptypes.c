...
size_t GetCipDataTypeLength(EipUint8 type, const EipUint8 *data) {
	...
	switch (type) {
		...
		case kCipStringN:
			if(NULL != data){
				length = (size_t)GetUintFromMessage(&data) + 2U; // string length + 2 bytes length indicator
			}
			break;

		case kCipShortString:
			if(NULL != data){
				length = (size_t)GetUsintFromMessage(&data) + 1U; // string length + 1 byte length indicator
			}
			break;

		case kCipEpath:
			if(NULL != data){
				length = (size_t)GetUintFromMessage(&data) + 2U; // path size + 2 bytes path size indicator
			}
			break;
