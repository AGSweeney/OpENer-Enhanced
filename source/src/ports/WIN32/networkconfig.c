...
static DWORD ConvertToIndexFromFakeAlias(const char *iface,
  ...
  errno_t conversion_result = mbstowcs_s(&converted_chars, p_if_alias, wc_cnt,
                                         iface, _TRUNCATE);
...
static DWORD WideToCipString(const WCHAR *const src,
  ...
  if(num_chars) {
    ...
    const size_t cnv_chars = wcstombs(buf, src, buffer_size);
    if(cnv_chars != num_chars) {
      FREE(buf);
      return ERROR_NO_UNICODE_TRANSLATION;
    }
  }
  ...
  dest->length = (EipUint16)num_chars;
  dest->string = buf;
