                                              ...
                                              CipTcpIpInterfaceConfiguration *iface_cfg)
{
...

  {
    ...
    if(ioctl(fd, SIOCGIFADDR, &ifr) == 0) {
      ipaddr = (CipUdint)( (struct sockaddr_in *) &ifr.ifr_addr )->sin_addr.s_addr;
    } else {
      close(fd);
      return kEipStatusError;
    }
    ...
    if(ioctl(fd, SIOCGIFNETMASK, &ifr) == 0) {
      netaddr = (CipUdint)( (struct sockaddr_in *) &ifr.ifr_netmask )->sin_addr.s_addr;
    } else {
      close(fd);
      return kEipStatusError;
    }
                                     ...
                                     CipTcpIpInterfaceConfiguration *iface_cfg)
{
  ...
  CipUdint gateway = (CipUdint)tmp_gw;
...
static EipStatus GetDnsInfoFromResolvConf(
  ...
  if(file_handle) {
    ...
    long file_length_result = ftell(file_handle);
    if(file_length_result < 0) {
      fclose(file_handle);
      return kEipStatusError;
    }
    file_length = (size_t)file_length_result;
