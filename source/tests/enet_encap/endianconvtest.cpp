...
TEST(EndianConversion, MoveMessageNOctetsClampsAtBufferEnd) {
  ENIPMessage message;
  InitializeENIPMessage(&message);

  MoveMessageNOctets((int)PC_OPENER_ETHERNET_BUFFER_SIZE + 1, &message);

  POINTERS_EQUAL(message.message_buffer + PC_OPENER_ETHERNET_BUFFER_SIZE,
                 message.current_message_position);
  LONGS_EQUAL(PC_OPENER_ETHERNET_BUFFER_SIZE, message.used_message_length);
}
...
TEST(EndianConversion, AddDintToMessageDoesNotOverflowBuffer) {
  ENIPMessage message;
  InitializeENIPMessage(&message);
  message.used_message_length = PC_OPENER_ETHERNET_BUFFER_SIZE - 2;
  message.current_message_position = message.message_buffer + message.used_message_length;

  AddDintToMessage(0x12345678, &message);

  LONGS_EQUAL(PC_OPENER_ETHERNET_BUFFER_SIZE - 2, message.used_message_length);
  POINTERS_EQUAL(message.message_buffer + PC_OPENER_ETHERNET_BUFFER_SIZE - 2,
                 message.current_message_position);
}
